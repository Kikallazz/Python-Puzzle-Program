...
if __name__ == "__main__":
    """
        its ya boi, the main function, you already know what does
    """
    ...
    os.system("python cropper.py --image test.jpg -f cropper_test\\raw\ --save cropper_test\cropped\\")
    ...
    if args["piece"].find('img0.jpg') != -1:
        path = ''
        for i in args["piece"][:args["piece"].find('img0.jpg')]:
            path += i
    ...
    if path != '':
        ...
        f = [x.decode('utf-8') for x in os.listdir(directory)]
        if 'img1.jpg' in f:
            for filename in os.listdir(directory):
                if filename.decode('utf-8') == 'img0.jpg':
                    pass
                pathIMG = path + filename.decode('utf-8')
                piece = cv2.imread(pathIMG, -1)
                main(canvas, piece)
